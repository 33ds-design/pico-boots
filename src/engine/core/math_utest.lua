...
describe('vector (boundary precision)', function ()

  describe('huge values', function ()
    it('vector(huge, huge) + vector(huge, huge) => vector(huge, huge)', function ()
      assert.are_same(vector(huge, huge), vector(huge, huge) + vector(huge, huge))
    end)
    it('vector(-huge, 0) * 2 => vector(-huge, 0)', function ()
      assert.are_same(vector(-huge, 0), vector(-huge, 0) * 2)
    end)
  end)

  describe('near-zero values', function ()
    it('vector(1e-9, 1e-9):is_zero() should be false', function ()
      assert.is_false(vector(1e-9, 1e-9):is_zero())
    end)
  end)

end)

describe('tile_vector / sprite_id_location (boundary coordinates)', function ()

  describe('to_sprite_id boundary', function ()
    it('sprite_id_location(0, 0) => 0', function ()
      assert.are_equal(0, sprite_id_location(0, 0):to_sprite_id())
    end)
    it('sprite_id_location(15, 15) => 255 (max on 16x16 spritesheet)', function ()
      assert.are_equal(255, sprite_id_location(15, 15):to_sprite_id())
    end)
    it('sprite_id_location(-1, -2) => -33 (negative coordinates)', function ()
      assert.are_equal(-33, sprite_id_location(-1, -2):to_sprite_id())
    end)
  end)

end)

describe('signed_speed_to_dir (boundary)', function ()
  it('should 1e-9 => right (tiny positive speed)', function ()
    assert.are_equal(horizontal_dirs.right, signed_speed_to_dir(1e-9))
  end)
  it('should -1e-9 => left (tiny negative speed)', function ()
    assert.are_equal(horizontal_dirs.left, signed_speed_to_dir(-1e-9))
  end)
end)

describe('oppose_dir (boundary / diagonal-range)', function ()
  it('should be involutive: oppose_dir(oppose_dir(dir)) == dir for all dirs', function ()
    assert.are_equal(directions.left, oppose_dir(oppose_dir(directions.left)))
    assert.are_equal(directions.right, oppose_dir(oppose_dir(directions.right)))
    assert.are_equal(directions.up, oppose_dir(oppose_dir(directions.up)))
    assert.are_equal(directions.down, oppose_dir(oppose_dir(directions.down)))
  end)
  it('should wrap around: oppose_dir(4) == oppose_dir(0) (out-of-range high)', function ()
    assert.are_equal(oppose_dir(0), oppose_dir(4))
  end)
  it('should wrap around: oppose_dir(-1) == oppose_dir(3) (out-of-range low)', function ()
    assert.are_equal(oppose_dir(3), oppose_dir(-1))
  end)
end)
